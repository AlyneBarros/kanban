...
function Header({ uniqueOrderCount, startedOrderCount, waitingOrderCount, setCustomSizes, setDefaultSizes, defaultSizes, genomTitle, areaTitle }) {
  ...
  useEffect(() => {
    if (autoScrollEnabled) {
      const intervalId = setInterval(scrollContainerHorizontal, 100 / scrollSpeed);
        ...
        <header className="flex justify-between dark:text-white items-center">
          ...
          <div className="flex space-x-4 items-center">
            <div className="flex items-center space-x-4">
              <div className="flex items-center">
                <span className="text-sl text-gray-600 dark:text-gray-300 ml-1">{uniqueOrderCount} ordens</span>
              </div>
              <div className="flex items-center">
                <span className="text-sl text-green-500 ml-1">{startedOrderCount} OP iniciadas</span>
              </div>
              <div className="flex items-center">
                <span className="text-sl text-orange-500 ml-1">{waitingOrderCount} OP em espera</span>
              </div>
            </div>
            <Menu as="div" className="relative">
              <Menu.Button className="flex items-center space-x-6">
                <img src={settingsIcon} alt="Settings" className={`h-10 w-10 ${darkSide ? 'filter brightness-0 invert' : ''}`} />
              </Menu.Button>
              <Transition
                as={Fragment}
                ...
                <Menu.Items className="absolute right-0 mt-2 w-72 origin-top-right bg-white dark:bg-[#2b2c37] divide-y divide-gray-100 rounded-md shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                  <div className="p-5 space-y-2">
                    ...
                    <div className="p-2 space-y-2">
                      ...
                      <button onClick={() => handleChangeSizes("350px", "350px", "89vh")} className="w-full text-left px-4 py-2 text-sm text-gray-900 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-gray-900 dark:hover:text-gray-300">Coluna Média</button>
                      <button onClick={() => handleChangeSizes("400px", "400px", "90vh")} className="w-full text-left px-4 py-2 text-sm text-gray-900-300 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-gray-900 dark:hover:text-gray-300">Coluna Grande</button>
                      <button onClick={() => handleChangeSizes("450px", "450px", "95vh")} className="w-full text-left px-4 py-2 text-sm text-gray-900 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-gray-900 dark:hover:text-gray-300">Coluna Extra Grande</button>
